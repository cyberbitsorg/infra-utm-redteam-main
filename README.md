...
This repo is deliberately narrow. It builds **one VM and nothing else**:

- No vulnerable targets and no isolated lab segment — for that, see the
  sibling [`infra-utm-redteam-lab`](../infra-utm-redteam-lab), a multi-VM lab
  (Kali attacker + intentionally vulnerable targets on their own network).
- No anonymity/egress stack (WireGuard, Mullvad, nftables kill-switch,
  gateway VM) — for that, see the sibling
  [`infra-utm-anon-egress`](../infra-utm-anon-egress), a Kali workstation
  behind a dedicated anonymity gateway.

Both siblings' provisioning pattern (UTM AppleScript → cloud-init → Ansible)
is reused here, just pointed at a single, general-purpose attack box instead
of a lab or a gateway.
...
See `docs/superpowers/specs/2026-08-04-utm-redteam-main-design.md` for the
full design if you want the reasoning behind these choices (that directory is
gitignored, so it only exists in a checkout that produced it).

## Requirements

- **Apple Silicon Mac** (ARM64) — the base image is `aarch64`, it will not
  boot on Intel
...
- `NET_MODE=bridged` puts the box directly on your real LAN with its own DHCP
  address — LAN-facing tools (ARP spoofing, LLMNR/NBNS poisoning, inbound
